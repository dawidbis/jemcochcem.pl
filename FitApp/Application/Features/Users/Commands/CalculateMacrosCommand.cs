...
public record CalculateMacrosCommand(Guid UserId) : IRequest<MacroResultDto>;
...
public class CalculateMacrosHandler : IRequestHandler<CalculateMacrosCommand, MacroResultDto>
{
    ...
    public async Task<MacroResultDto> Handle(CalculateMacrosCommand request, CancellationToken ct)
    {
        ...
        // 1. Najpierw liczymy zapotrzebowanie na kalorie (TDEE) używając mnożnika z bazy
        var tdee = _tdeeService.CalculateTDEE(user.Weight, user.Height, user.Age, user.Gender, user.ActivityMultiplier);
